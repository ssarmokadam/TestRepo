...
public class ExtJsTargetAdapter implements TranslationTarget {
	...
	public void generateStream(Map<String, Node> root, OutputStream out) {
		...
		System.out
				.println("Complete js String is -- "
						+ this.prefix.append(createJsString(root)).append(
								this.postfix));
	}
	...
	public void generateFile(Map<String, Node> root, File out) {
		...
		try {
			...
			bw.write(this.prefix.append(createJsString(root))
					.append(this.postfix).toString());
			bw.close();
		} catch (IOException e) {
		...
		for (String key : keySet) {
			...
			if (node.getText() == null && node.getChildren().size() == 1) {
				...
				if (this.completeJsString.toString().isEmpty()) {
					this.completeJsString.append(this.startJsStringBuffer)
							.append(this.endJsStringBuffer);
				} else {
					this.completeJsString.append(",")
							.append(this.startJsStringBuffer)
							.append(this.endJsStringBuffer);
				}
				this.startJsStringBuffer = new StringBuffer();
				this.endJsStringBuffer = new StringBuffer();
			} else {
				System.out.println("ERROR:: This version of devon-locale supports only simple proprties. "+key+" This property will be ignored.");
			}
